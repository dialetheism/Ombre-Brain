...
def test_dynamic_list_and_search_fail_closed_on_noncanonical_persisted_title_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f10000000001"
    invalid_id = "f10000000002"
    _write_record(
        root,
        layer="dynamic",
        memory_id=valid_id,
        body="valid noncanonical title search sentinel",
    )
    _write_record(
        root,
        layer="dynamic",
    ...
    with pytest.raises(
        MemoryFormatError,
        match=r"^stored Phase 1 title is not canonical$",
    ):
        library.list()
    ...
    with pytest.raises(
        ...
        library.search("valid noncanonical title search sentinel")

    assert _snapshot_files(root) == before


def test_archive_list_and_search_fail_closed_on_noncanonical_persisted_title_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f20000000001"
    invalid_id = "f20000000002"
    _write_record(
        root,
        layer="archive",
        memory_id=valid_id,
        body="valid archive noncanonical title search sentinel",
        metadata_updates={"type": "archived"},
    )
    _write_record(
        root,
        ...
        memory_id=invalid_id,
        body="Valid body",
        metadata_updates={"name": " leading", "type": "archived"},
    )
    before = _snapshot_files(root)
    library = MemoryLibrary(root, allow_existing_nonempty=True)
    ...
    with pytest.raises(
        ...
        library.search(
            "valid archive noncanonical title search sentinel",
            archived=True,
        )
...
def test_dynamic_list_and_search_fail_closed_on_invalid_persisted_title_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f30000000001"
    invalid_id = "f30000000002"
    _write_record(
        root,
        layer="dynamic",
        memory_id=valid_id,
        body="valid invalid title search sentinel",
    )
    _write_record(
        root,
        layer="dynamic",
    ...
    with pytest.raises(
        ...
        library.search("valid invalid title search sentinel")

    assert _snapshot_files(root) == before


def test_archive_list_and_search_fail_closed_on_invalid_persisted_title_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f40000000001"
    invalid_id = "f40000000002"
    _write_record(
        root,
        layer="archive",
        memory_id=valid_id,
        body="valid archive invalid title search sentinel",
        metadata_updates={"type": "archived"},
    )
    _write_record(
        root,
    ...
    with pytest.raises(
        ...
        library.search(
            "valid archive invalid title search sentinel",
            archived=True,
        )
...
def test_dynamic_list_and_search_fail_closed_on_missing_persisted_tags_without_rewrite(
    ...
    root = _isolated_root(tmp_path)
    valid_id = "f50000000001"
    invalid_id = "f50000000002"
    _write_record(
        root,
        layer="dynamic",
        memory_id=valid_id,
        body="valid missing tags search sentinel",
    )
    _write_record(
        root,
        layer="dynamic",
    ...
    with pytest.raises(
        MemoryFormatError,
        match=r"^stored Phase 1 tags must be a list$",
    ):
        library.list()
    ...
    with pytest.raises(
        ...
        library.search("valid missing tags search sentinel")

    assert _snapshot_files(root) == before


def test_archive_list_and_search_fail_closed_on_missing_persisted_tags_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f60000000001"
    invalid_id = "f60000000002"
    _write_record(
        root,
        layer="archive",
        memory_id=valid_id,
        body="valid archive missing tags search sentinel",
        metadata_updates={"type": "archived"},
    )
    _write_record(
        root,
    ...
    with pytest.raises(
        ...
        library.search(
            "valid archive missing tags search sentinel",
            archived=True,
        )
...
def test_dynamic_list_and_search_fail_closed_on_invalid_persisted_tags_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "f70000000001"
    invalid_id = "f70000000002"
    _write_record(
        root,
        layer="dynamic",
        memory_id=valid_id,
        body="valid invalid tags search sentinel",
    )
    _write_record(
        root,
        layer="dynamic",
    ...
    with pytest.raises(
        ...
        library.search("valid invalid tags search sentinel")
...
def test_archive_list_and_search_fail_closed_on_noncanonical_persisted_tags_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "fa0000000001"
    invalid_id = "fa0000000002"
    _write_record(
        root,
        layer="archive",
        memory_id=valid_id,
        body="valid archive noncanonical tags search sentinel",
        metadata_updates={"type": "archived"},
    )
    _write_record(
        root,
        ...
        memory_id=invalid_id,
        body="Valid body",
        metadata_updates={"type": "archived", "tags": ["duplicate", "duplicate"]},
    )
    before = _snapshot_files(root)
    library = MemoryLibrary(root, allow_existing_nonempty=True)
    ...
    with pytest.raises(
        ...
        library.search(
            "valid archive noncanonical tags search sentinel",
            archived=True,
        )
...
def test_archive_list_and_search_fail_closed_on_invalid_persisted_id_without_rewrite(
    tmp_path: Path,
) -> None:
    root = _isolated_root(tmp_path)
    valid_id = "fc0000000001"
    invalid_id = "fc0000000002"
    _write_record(
        root,
        layer="archive",
        memory_id=valid_id,
        body="valid archive invalid persisted id search sentinel",
        metadata_updates={"type": "archived"},
    )
    _write_record(
        root,
        ...
        memory_id=invalid_id,
        body="Valid body",
        metadata_updates={"type": "archived", "id": "not-a-valid-id"},
    )
    before = _snapshot_files(root)
    library = MemoryLibrary(root, allow_existing_nonempty=True)
    ...
    with pytest.raises(
        ...
        library.search(
            "valid archive invalid persisted id search sentinel",
            archived=True,
        )
